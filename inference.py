"""
Inference script for SSL-Medical-Segmentation
Supports both 2D and 3D semi-supervised segmentation checkpoints
...
def run_inference(args, config):
    ...
    """
    print("\n" + "="*80)
    print("SSL-Medical-Segmentation - Inference")
    print("="*80 + "\n")
    ...
    # Calculate average metrics and standard deviation
    if all_metrics:
        print("\n" + "="*80)
        ...
        with open(metrics_file, 'w', newline='') as f:
            ...
            avg_row = {'filename': 'MEAN'}
            std_row = {'filename': 'STD'}
            for key in all_metrics[0].keys():
                if key != 'filename':
                    avg_row[key] = f"{avg_metrics[key]:.4f}"
                    std_row[key] = f"{std_metrics[key]:.4f}"
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='SSL-Medical-Segmentation Inference')
