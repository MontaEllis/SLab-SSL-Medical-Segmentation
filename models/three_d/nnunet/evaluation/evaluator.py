...
def aggregate_scores(test_ref_pairs,
    ...
    for i in range(len(all_res)):
        ...
        # append score list for mean and std
    ...
    for label in all_scores["mean"]:
        for score in all_scores["mean"][label]:
            values = all_scores["mean"][label][score]
            if nanmean:
                all_scores["mean"][label][score] = float(np.nanmean(values))
                all_scores["std"][label][score] = float(np.nanstd(values))
            else:
                all_scores["mean"][label][score] = float(np.mean(values))
                all_scores["std"][label][score] = float(np.std(values))
...
def aggregate_scores_for_experiment(score_file,
    ...
    json_dict["task"] = json_task
    json_dict["author"] = json_author
    json_dict["results"] = {"all": results, "mean": results_mean, "std": results_std}
    json_dict["id"] = hashlib.md5(json.dumps(json_dict).encode("utf-8")).hexdigest()[:12]
    if json_output_file is not None:
